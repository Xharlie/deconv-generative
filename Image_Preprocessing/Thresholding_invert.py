...
TARGET_FOLDER_PREFIX = '../chair_segmented_invert/'
...
def load_and_crop_write(folder, counter):
  ...
  for filename in os.listdir(folder):
    ...
    if img is not None:
      ret, img_threshold = cv2.threshold(img, 254, 255, cv2.THRESH_BINARY_INV)
      ...
      with open(LABEL_FOLDER_PREFIX + "segm_invert.txt", "a") as label_file:
        label_file.write('chair_segmented_invert/' + str(counter) + '_' + filename + " " + str(counter) + "\n")
